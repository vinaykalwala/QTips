...
def product_deals(request,slug,deal_id,banner_id):
    ...
    for product in products:
        ...
        # Append the modified product dictionary to the list
...
def index(request):
    ...
    if top_Featured_L:
        ...
        top_Featured_R = []
    # Fetch products more efficiently
    ...
    # Fetch 10 products for each section


    deal = Deal_of_the_day.objects.first()  # You can adjust this to fetch the correct deal
    topDealsOfTheDay = Deal_of_the_day.objects.all()[:2]
    countdown_datetime = deal.deal_datetime.strftime("%b %d %Y %H:%M:%S")
    ...
    params = {
        ...
        'sliders':sliders,
        # 'partners': Partners.objects.all().order_by('-id'),  # Fetch directly in the params if not used elsewhere
...
def contact(request):
    ...
    return render(request,'../templates/Main/contact.html',context=params)
...
def PRODUCTS(request, sub_category_):
    ...
    filtered_products = products.filter(color_conditions | brand_conditions | size_conditions).distinct()
...
def prod_detail(request, slug,product_id=None, variant_id=None, color=None, size=None):
    ...
    if product_id:
        ...
        product = get_object_or_404(Product, slug=slug, variants__id=variant_id)  # Fix: Use double underscores for related field
    else:
    ...
    variant_details = [
        {
            ...
            'price': variant.price,
            'image_url': variant.image_id if variant and variant.image_id else '',
        }
    ]
    ...
    color_filter = variants.filter(size__name=default_size)
    unique_colors = []
    for i in color_filter:
        unique_colors.append(i.color)
    color_radio = variant.color.name if variant.color else ''
    size_radio = variant.size.name if variant.size else ''
    # print(unique_colors)
    default_color = unique_colors[0] if unique_colors else ''
    selected_size = request.GET.get('size', default_size)
    selected_color = request.GET.get('color', default_color)
    ...
    context.update({
        ...
        'color_radio': selected_color,  # Set default color value
        'size_radio': selected_size,
    })
    ...
    if request.method == 'GET':
        ...
        print(v_size,v_color,"____________")
...
def My_Account(request):
    ...
    return render(request,'../templates/Account/My_account.html',context=params)
